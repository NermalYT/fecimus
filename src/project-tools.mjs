...
async function captureGit(argv, root, options, maxChars) {
  ...
  return await new Promise(resolve => {
    ...
    const finish = (exitCode, error) => {
      ...
      resolve({ text: output, exit_code: exitCode, truncated, ...(reason ? { reason } : {}), ...(error ? { error: error.message } : stderr.trim() ? { [exitCode === 0 || (exitCode === null && reason === 'output limit') ? 'warning' : 'error']: stderr.trim() } : {}) });
    };
