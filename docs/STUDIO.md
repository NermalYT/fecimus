...
| Godot | Project/GDScript/C# edits; Linux headless jobs and editor launch | Godot 4.7.2 Linux headless import/run/edit verified; GUI, C#, exports and other platforms unverified ([evidence](PERFORMANCE.md#fecimus-3-real-godot-headless-verification)) |
...
[Godot's command-line interface](https://docs.godotengine.org/en/stable/tutorials/editor/command_line_tutorial.html) provides headless execution. For an installed matching editor and an existing project, a bounded import job can use `command:"godot"`, `args:["--headless","--path","/absolute/project","--editor","--quit"]`, and that project as `cwd`. Match flags to the installed version; exit code and logs must be checked. This is an invocation template. The separate [Godot validation](PERFORMANCE.md#fecimus-3-real-godot-headless-verification) covers a small GDScript headless project on Linux only.
